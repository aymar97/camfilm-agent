...
# ---------------------------------------------------------------------
# OPTIMISATION : Retries augmentés + attente progressive pour contrer les 503
# ---------------------------------------------------------------------
def _generate_with_retry(client, prompt: str, config=None, max_retries: int = 5):
    """
    Appelle Gemini avec tentatives automatiques.
    En cas de 503 (surcharge) ou 429 (quota), attend progressivement puis réessaie.
    """
    last_error = None
    for attempt in range(max_retries):
        try:
            ...
            ) if config else client.models.generate_content(model=GEMINI_MODEL, contents=prompt)
...
LOCAL_TOOL_SUMMARY = [
    {
        "name": "analyze_scene",
        "description": "Analyse une scène de tournage au Cameroun : alertes IA, risques culturels, administratifs, logistiques, recommandations linguistiques.",
        "arguments_example": {
            "scene": {
    ...
    },
    {
        ...
        "description": "Estime un budget de tournage au Cameroun en XAF à partir des datasets locaux.",
        "arguments_example": {
            "jours_tournage": 2, "equipe_personnes": 20, "camera": "Sony FX3",
            "son": "Kit son complet", "eclairage": "Kit LED 3 panneaux",
            "generateur": "Groupe diesel moyen", "catering": "Mama du quartier",
            "lieu_type": "village", "taille_chefferie": "village_moyen",
            "marche": False, "transport_generateur": True,
            "zone_transport": "yaounde_peripherie_50km",
            "autorisation_minac": False, "fixer_local": True
        }
    },
    {
        "name": "camerounize_dialogue",
        "description": "Camérounise un dialogue en proposant du pidgin english, du camfranglais ou des expressions locales selon le contexte.",
        "arguments_example": {
    ...
    },
    {
        ...
        "description": (
            ...
            "dialogue, storyboard et post-production en un seul appel. "
            "À utiliser dès que la demande contient plusieurs éléments."
        ),
        "arguments_example": {
            ...
            "budget_params": {
                "jours_tournage": 3, "equipe_personnes": 15,
                "camera": "Sony FX3", "lieu_type": "village"
            }
        }
    },
    {
        "name": "generate_storyboard",
        "description": "Génère un storyboard textuel de tournage : liste de plans à tourner, durée par plan, type de son recommandé, et recommandations pour la phase de réalisation.",
        "arguments_example": {
            "scene": {
    ...
    },
    {
        ...
        "description": "Fournit des recommandations techniques de post-production : étalonnage (LUTs pour peaux africaines), nettoyage sonore (vent, marché), sound design, et recommandations de prestataires à Yaoundé/Douala.",
        "arguments_example": {
            "scene_type": "marche_exterieur",
            "camera": "Sony FX3",
            "audio_issues": ["vent", "bruit de fond"]
        }
    }
]

KNOWN_TOOLS = {
    "analyze_scene", "estimate_budget", "camerounize_dialogue",
    "build_llm_context", "generate_storyboard", "post_production_advice"
}
...
def parallel_search(query: str) -> Dict[str, Any]:
    ...
    if not PARALLEL_API_KEY:
        return {
            "configured": False, "ok": False, "query": query,
            "note": "Parallel Search API n'est pas encore configuré. Ajoute PARALLEL_API_KEY dans .env."
        }
    try:
        ...
        return {
            "configured": True, "ok": True, "query": query,
            "sdk": "parallel-web (official)",
            "results_count": len(results_serializable),
            "data": {"results": results_serializable, "search_id": getattr(search, "search_id", None)},
        }
    except Exception as exc:
...
def gcp_agent_builder_query(query: str) -> Dict[str, Any]:
    ...
    if not GCP_AGENT_BUILDER_URL:
        return {
            "configured": False, "ok": False, "query": query,
            "note": "Google Cloud Agent Builder n'est pas encore configuré."
        }
    ...
    try:
        with httpx.Client(timeout=30) as client:
            response = client.post(
                ...
                json={"input": query, "query": query, "message": query},
                headers=headers,
            )
        return {
            "configured": True, "ok": response.status_code < 400,
            "status_code": response.status_code, "query": query,
            "data": response.json() if response.status_code < 400 else {"raw": response.text}
        }
    except Exception as exc:
...
# ---------------------------------------------------------------------
# OPTIMISATION : Nouvelle règle pour forcer build_llm_context
# ---------------------------------------------------------------------
def _build_planning_prompt(message: str, context: Dict[str, Any]) -> str:
    ...
    return (
        ...
        "Règles importantes :\n"
        "- RÈGLE PRIORITAIRE : Si la demande contient PLUSIEURS éléments (ex: budget ET storyboard, analyse ET budget, préparation complète) : utilise IMPÉRATIVEMENT build_llm_context pour tout traiter d'un seul coup.\n"
...
def _build_final_prompt(message: str, plan: Dict[str, Any], tool_result: Optional[Dict[str, Any]], web_result: Optional[Dict[str, Any]], gcp_result: Optional[Dict[str, Any]], language: str = "Français") -> str:
    lang_instruction = (
        ...
        else "Tu dois répondre en français, de manière claire, structurée et utile pour un producteur camerounais."
    )
    return (
        ...
        "4. Recommandations terrain / Storyboard / Post-production\n"
...
def run_agent(message: str, context: Dict[str, Any], datasets: Dict[str, Any], execute_tool_fn: Callable[..., Dict[str, Any]], use_parallel: bool = True, use_gcp: bool = False) -> Dict[str, Any]:
    if not GEMINI_API_KEY:
        return {"status": "error", "error": "GEMINI_API_KEY manquant dans .env", "agent_status": agent_status()}

    message = str(message or "").strip()
    if not message:
        return {"status": "error", "error": "Le message utilisateur est vide."}

    language = str((context or {}).get("language", "Français")).strip() or "Français"
    client = genai.Client(api_key=GEMINI_API_KEY)

    # -----------------------------------------------------------------
    # 1. Planification : Gemini choisit l'outil
    # -----------------------------------------------------------------
    try:
        planning_response = _generate_with_retry(
            client,
            _build_planning_prompt(message, context),
    ...
    if tool != "final_answer":
        ...
        if tool == "post_production_advice" and context:
            if not arguments.get("scene_type") and isinstance(context.get("lieu"), dict):
                arguments["scene_type"] = context.get("lieu", {}).get("type", "général")
            if not arguments.get("camera") and isinstance(context.get("budget_params"), dict):
                arguments["camera"] = context.get("budget_params", {}).get("camera", "Standard")

        try:
            tool_result = execute_tool_fn(name=tool, arguments=arguments, datasets=datasets)
        except Exception as exc:
            tool_result = {"status": "error", "tool": tool, "error": str(exc)}

    # -----------------------------------------------------------------
    # 5. Réponse finale par Gemini
    # -----------------------------------------------------------------
    try:
        final_response = _generate_with_retry(
            client,
            _build_final_prompt(message, plan, tool_result, web_result, gcp_result, language),
        ...
        if "503" in str(exc) or "UNAVAILABLE" in str(exc):
            final_text = (
                "⚠️ Gemini est actuellement surchargé (erreur 503), mais les données ci-dessous "
                "(alertes, budget, storyboard) ont bien été calculées. "
                "Veuillez réessayer dans quelques secondes pour le rapport complet."
            )
        else:
