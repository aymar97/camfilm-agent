...
Le sélecteur change la langue de l'interface ET du rapport de l'agent,
mais les DONNEES (datasets, alertes, budgets, dialogues) restent en français :
c'est la richesse culturelle du projet.
"""
...
# ---------------------------------------------------------------------
# Traductions de l'interface (les DONNEES restent en français)
# ---------------------------------------------------------------------
...
TEXTS = {
    "Français": {
        ...
        "error_connect": "Impossible de se connecter à l'API FastAPI. Assurez-vous que le serveur tourne sur http://127.0.0.1:8000",
    ...
    "English": {
        ...
        "error_connect": "Cannot connect to the FastAPI API. Make sure the server runs on http://127.0.0.1:8000",
...
def call_agent(message: str, context: Dict[str, Any], language: str) -> Dict[str, Any]:
    """Appelle l'API FastAPI avec Gemini, en passant la langue choisie."""
    payload = {
        "message": message,
    ...
    try:
        with httpx.Client(timeout=60.0) as client:
            response = client.post(
                f"{API_URL}/hackathon/agent/chat/gemini",
                json=payload,
        ...
        return {"status": "error", "error": "CONNECT"}
...
# Sélecteur de langue (tout en haut de la sidebar)
    ...
    if not message:
        ...
        with st.spinner(tr["analyzing"]):
            ...
            if result.get("status") == "error":
                ...
                # Protection contre tool_result = None (l'agent peut choisir de ne pas appeler d'outil)
